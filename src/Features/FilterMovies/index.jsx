...
function FilterMovies({ filters, setFilters, closeFilter }) {
    ...
    <div className="bg-black text-black bg-opacity-50 flex justify-center items-center w-full">
      <div className="bg-white p-6 rounded-md w-full">
          ...
          {genres.map((g) => (
            <label key={g.id} className="flex items-center space-x-2">
              <input
                type="checkbox"
                value={g.id}
                checked={tempFilters.genres.includes(g.id)}
                onChange={(e) => {
                  setTempFilters((prev) => ({
                    ...prev,
        ...
        <div className="flex justify-between mt-4">
          <button className="bg-gray-400 px-4 py-2 rounded-md" onClick={closeFilter}>
            Đóng
          </button>
          <button
